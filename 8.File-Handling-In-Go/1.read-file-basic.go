...
func readStreamFashion(filename string) {
	...
	fmt.Println("File content (streamed):")
	for {
...
// Stream Based Reading => via bufio package
func readStreamWithBufio(filename string) {
	file, err := os.Open(filename)
	if err != nil {
		fmt.Println("Error opening file:", err)
		return
...
func bufferRead(filename string){
	...
	buffer := make([]byte, 100) // size is 100bytes
	bufferedLength, err := fileObj.Read(buffer)
	if err != nil {
		panic(err)
	}
	println("content: ", string(buffer), bufferedLength)
}
